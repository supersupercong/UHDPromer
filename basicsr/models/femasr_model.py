...
class FeMaSRModel(BaseModel):
    ...
    def optimize_parameters(self, current_iter):
        ...
        self.output, self.SRNet_output = self.net_g(self.lq)
        ...
        if self.opt['network_g']['SR_Guidance']:
            l_pix_SRNet_output = self.l1(self.SRNet_output, self.gt)
            l_g_total = l_g_total + 0.5 * l_pix_SRNet_output

            loss_dict['l_pix_SRNet_output'] = l_pix_SRNet_output
        ...
        if train_opt.get('fft_opt', None):
            ...
            if self.opt['network_g']['SR_Guidance']:
                l_fft_SRNet_output = self.cri_fft(self.SRNet_output, self.gt)
                l_g_total = l_g_total + 0.5 * l_fft_SRNet_output
                loss_dict['l_freq_SRNet_output'] = l_fft_SRNet_output
